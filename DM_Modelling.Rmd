...
# load data
SZ_model = readRDS("posterior_mixture_models/DA_SZ.rds")
HC_model = readRDS("posterior_mixture_models/DA_HC.rds")
...
summaries$group = ifelse(summaries$group == "SCZ", "Schizophrenia Subjects (SCZ)", 
                         "Healthy Controls Subjects (HC)")

summaries$group <- factor(summaries$group, levels = c("Schizophrenia Subjects (SCZ)", "Healthy Controls Subjects (HC)"))
...
ggplot(summaries, aes(x=theta, y=estimate, fill = theta)) + 
  ...
  labs(title = "Mixing Proportions of SCZ and HC against Double Alternation Opponent", 
